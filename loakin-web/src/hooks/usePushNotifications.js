 ...
 * Silently skips if the browser doesn't support it (e.g. some mobile browsers).
 * Flutter will bypass this entirely and call POST /push/fcm-token instead.
 */
export function usePushNotifications(loggedIn) {
    useEffect(() => {
        const vapidKey = import.meta.env.VITE_VAPID_PUBLIC_KEY;

        if (!loggedIn || !vapidKey || !('serviceWorker' in navigator) || !('PushManager' in window)) {
            return;
        }
